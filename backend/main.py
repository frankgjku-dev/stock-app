...
def _yf_info_sync(sym: str) -> dict:
    """抓 yfinance Ticker.info（sector / industry）"""
...
@app.get("/api/stocks/{symbol}/industry")
async def stock_industry(symbol: str):
    """獨立產業資訊端點（避免阻塞 analyze）"""
    try:
        loop    = asyncio.get_event_loop()
        ...
        return {
            "sector":   _SECTOR_ZH.get(sector_en,   sector_en)   or None,
            "industry": _INDUSTRY_ZH.get(industry_en, industry_en) or None,
        }
    except Exception:
...
async def analyze_stock(symbol: str):
    ...
    try:
        ...
        return {
            ...
            # 產業熱度（純由 RS + 趨勢分數計算，不含 yfinance info）

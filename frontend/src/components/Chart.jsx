...
export default function Chart({ candles, indicators, activeTool, clearRef }) {
  ...
  // All mutable drawing state in one ref to avoid stale closures
  const S = useRef({
    chart: null, series: {},
    drawings: [],   // finished drawings
    preview: null,  // in-progress drawing
    selectedIdx: -1,
  })
  ...
  function toData(px, py) {
    ...
    const price = series.candle.coordinateToPrice(py)
    return (time != null && price != null) ? { time, price } : null
  }

  /* ─── canvas sizing (NO devicePixelRatio scaling to avoid coord bugs) ─ */
  function syncCanvas() {
    const el = canvasRef.current
    const ct = containerRef.current
    if (!el || !ct) return
    // Only update if size actually changed (avoids clearing mid-draw)
    if (el.width !== ct.clientWidth || el.height !== ct.clientHeight) {
  ...
  function redraw() {
    ...
    drawings.forEach((d, i) => paint(ctx, d, canvas.width, canvas.height, null, i === selectedIdx))
    if (preview) paint(ctx, preview, canvas.width, canvas.height, null, false, true)
  }

  function paint(ctx, d, W, H, _mouse, selected, isPreview) {
    const { type, pts } = d
    if (!pts[0]) return
    const p1 = toPixel(pts[0].price, pts[0].time)
    if (!p1) return

    const color = selected ? DRAW_HOVER : DRAW_COLOR
    ctx.save()
    ...
    const p2raw = pts[1] ? toPixel(pts[1].price, pts[1].time) : d.cursor
    const p2    = p2raw || null
    ...
    switch (type) {
      case 'horizontal': {
        ...
        ctx.fillRect(W - tw - 14, p1.y - 11, tw + 10, 16)
        ctx.fillStyle = color
        ctx.fillText(lbl, W - tw - 9, p1.y + 2)
        break
      }
      case 'vertical': {
      ...
      case 'ray': {
        ...
        // anchor dots
        const anchors = pts[1]
          ? [p1, toPixel(pts[1].price, pts[1].time)].filter(Boolean)
          : [p1]
        ctx.setLineDash([])
        anchors.forEach(p => {
          ctx.beginPath(); ctx.arc(p.x, p.y, selected ? 5 : 3, 0, Math.PI * 2)
          ctx.fillStyle = color; ctx.fill()
          ctx.strokeStyle = 'rgba(248,240,224,0.9)'; ctx.lineWidth = 1.5; ctx.stroke()
        })
        break
      }
      case 'rectangle': {
        if (!p2) break
        ctx.fillStyle = selected ? 'rgba(224,144,64,0.10)' : 'rgba(184,110,42,0.07)'
  ...
  useEffect(() => {
    ...
    const chart = createChart(ct, {
      ...
      crosshair: {
        ...
        vertLine: { color: '#b86e2a55', labelBackgroundColor: '#b86e2a' },
        horzLine: { color: '#b86e2a55', labelBackgroundColor: '#b86e2a' },
      },
  ...
  function getCanvasXY(e) {
    ...
    // getBoundingClientRect gives CSS px; canvas.width is also CSS px → direct match
  ...
  function handlePointerDown(e) {
    ...
    if (!S.current.preview) {
      if (oneClick) {
        ...
        S.current.preview = { type: tool, pts: [dp], cursor: { x, y } }
      }
    } else {
      // second click → commit
      S.current.preview.pts.push(dp)
      S.current.drawings.push({ ...S.current.preview, cursor: null })
  ...
  function hitTest(d, mx, my) {
    ...
    // trendline / ray / fibonacci: distance to segment
    const dx=p2.x-p1.x, dy=p2.y-p1.y
    const len2=dx*dx+dy*dy
    if (len2===0) return Math.hypot(mx-p1.x,my-p1.y)<T
